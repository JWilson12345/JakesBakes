...
function emailMessage(type, payload, meta) {
  ...
  if (type === "preorder") {
    return [
      ...
      "Items",
      "-----",
      ...(payload.items || []).map((item) => `${item.quantity} x ${item.boxSize} Cookies - ${item.title || "The Biscoff Royale"} - ${money(item.price)} each`),
      "",
      "Notes",
      "-----",
...
function preorderCopy() {
  ...
  const map = {
    ...
    preparing: { bar: "This week's cookies are being prepared for Friday.", cta: "Collection info", href: "collection.html", product: "Preorders open Friday", canOrder: false }
  };
...
function productCatalog() {
  return {
    "biscoff-royale": {
      ...
      boxAlt: {
        four: "Open 4 Cookie Bag of The Biscoff Royale cookies with milky Nutella drizzle",
        six: "Open 6 Cookie Bag of The Biscoff Royale cookies with milky Nutella drizzle"
      },
...
function addToCart(size, productId = defaultProductId()) {
  ...
  else items.push({ id, productId: product.id, size, boxSize: count, title: product.boxTitle, price, quantity: 1 });
  saveCart(items);
  openCartDrawer();
}
...
function boxDetails(size, productId = defaultProductId()) {
  ...
  return {
    ...
    label: `${isFour ? "4" : "6"} Cookies selected${isFour ? "" : " - most popular"} - ${product.name}`
  };
}
...
function footerHtml() {
  ...
  return `<footer class="footer"><div class="container footer-grid">
    <div class="footer-brand">
      ...
      <p>Fresh every Friday. Collection only in Barnsley.</p>
...
function renderCartLines(container, compact = false) {
  ...
  if (!items.length) {
    container.innerHTML = compact
      ? `<div class="drawer-empty"><strong>Your cart is empty.</strong><p>Choose 4 or 6 cookies to start your preorder.</p><a class="button" href="${basePath()}shop.html">Choose your order</a></div>`
      : `<div class="note"><strong>Your cart is empty.</strong><p>Choose 4 or 6 cookies to start your preorder.</p><a class="button" href="${basePath()}shop.html">Choose your order</a></div>`;
    return;
  }
  container.innerHTML = `<div class="stack">${items.map((item) => `
    <article class="card pad cart-row ${compact ? "cart-row-compact" : ""}">
      ...
      <div class="cart-item-copy">
        <h3>${item.boxSize} Cookies${item.size === "six" ? ` <span class="popular-badge">Most popular</span>` : ""}</h3>
        <p>${item.title}</p>
        <p><strong>${money(item.price)}</strong> each</p>
        <div class="cart-actions">
          <div class="quantity" aria-label="Quantity for ${item.boxSize} cookies"><button type="button" data-qty-minus="${item.id}" aria-label="Reduce quantity">-</button><span>${item.quantity}</span><button type="button" data-qty-plus="${item.id}" aria-label="Increase quantity">+</button></div>
          <button class="button secondary cart-remove" type="button" data-remove="${item.id}">Remove</button>
        </div>
      </div>
    </article>`).join("")}
    ${compact ? `<div class="drawer-subtotal"><span>Subtotal</span><strong>${money(cartSubtotal())}</strong><small>Collection only from Barnsley.</small></div>` : `<div class="note"><strong>Subtotal: ${money(cartSubtotal())}</strong><p>Collection only from Barnsley. Exact instructions are included with your order details.</p></div>`}
  </div>`;
}
...
function renderReviewCarousels(list) {
...
function orderConfirmationInit() {
  ...
  target.innerHTML = `<div class="card pad stack"><h2>Request ${order.number}</h2>${order.items.map((item) => `<p>${item.quantity} x ${item.boxSize} Cookies - ${escapeHtml(item.title || "The Biscoff Royale")} - ${money(item.price)} each</p>`).join("")}<p><strong>Estimated subtotal: ${order.total}</strong></p><p>Collection date: ${order.collectionDate}</p><p>Collection slot: ${order.slot}</p><p>Jake will reply to ${escapeHtml(order.email)} to confirm availability, payment and collection details.</p><div class="cluster"><a class="button" href="collection.html">Collection information</a><a class="button secondary" href="ingredients-allergens.html">Allergen information</a><a class="button secondary" href="contact.html">Contact Jake</a></div></div>`;
}
...
function searchInit() {
  ...
  const pages = [
    ["Shop", "Choose The Biscoff Royale as 4 or 6 cookies.", "shop.html"],
