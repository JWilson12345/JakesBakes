...
function emailMessage(type, payload, meta) {
  ...
  if (type === "preorder") {
    return [
      ...
      "Items",
      "-----",
      ...(payload.items || []).map((item) => `${item.quantity} x ${item.boxSize} Cookie Box - ${item.title || "Cookie Box"} - ${money(item.price)} each`),
      "",
      "Notes",
      "-----",
...
function getState() {
  ...
  return normalizePreorderState(remoteSiteSettings?.preorderState || STORE_CONFIG.preorderState);
}
...
function productCatalog() {
  return {
    "biscoff-royale": {
      ...
      boxTitle: "The Biscoff Royale Cookie Box",
      url: "product.html",
      image: "assets/images/product-cookie.webp",
      boxImages: { four: "assets/images/four-box.webp", six: "assets/images/six-box.webp" },
      alt: "Brown-butter Biscoff cookie with Biscoff biscuit and Nutella-milk drizzle",
      boxAlt: {
        four: "White box of 4 Biscoff Royale cookies with Nutella-milk drizzle",
        six: "White box of 6 Biscoff Royale cookies with Nutella-milk drizzle"
      },
      summary: "A brown-butter chocolate chip cookie base topped with Biscoff biscuit, Nutella-milk drizzle and a milk chocolate chip.",
      detail: "The Biscoff Royale uses the same brown-butter chocolate chip base, then adds Biscoff biscuit on top with Nutella-and-milk drizzle and a milk chocolate chip.",
      badges: ["Biscoff topping", "Nutella-milk drizzle"]
    },
    "brown-butter-chocolate-chip": {
      id: "brown-butter-chocolate-chip",
      name: "Brown Butter Chocolate Chip",
      boxTitle: STORE_CONFIG.plainProductName || "Brown Butter Chocolate Chip Cookie Box",
      url: "chocolate-chip.html",
      image: "assets/images/chocolate-chip-cookie.webp",
      boxImages: { four: "assets/images/four-box-chocolate-chip.webp", six: "assets/images/six-box-chocolate-chip.webp" },
      alt: "Brown butter chocolate chip cookie with milk chocolate chunks",
      boxAlt: {
        four: "Brown Butter Chocolate Chip Cookie Box with 4 cookies",
        six: "Brown Butter Chocolate Chip Cookie Box with 6 cookies"
      },
      summary: "A handcrafted brown-butter cookie with Madagascan vanilla, milk chocolate chunks and milk chocolate chips.",
      detail: "A handcrafted brown butter chocolate chip cookie made with browned British butter, plain and strong white flour, dark brown sugar, Madagascan vanilla extract, milk chocolate chunks and milk chocolate chips.",
      badges: ["Browned British butter", "Milk chocolate"]
    }
  };
}
...
async function adminSetPreorderState(nextState, adminCode) {
  ...
  remoteSiteSettings = { preorderState: state };
  window.dispatchEvent(new Event("sitestatechange"));
  return state;
}
...
function boxDetails(size, productId = defaultProductId()) {
  ...
  return {
    ...
    label: `${isFour ? "4" : "6"} Cookie Box selected - ${product.name}`
  };
}
...
function headerHtml() {
  ...
  return `
    <div class="announcement">${state.bar} <a href="${p}shop.html">${state.cta}</a></div>
    ...
    <div class="mobile-panel" data-mobile-panel aria-hidden="true">
      <nav class="mobile-menu" id="mobile-menu" aria-label="Mobile navigation">
        ...
        <a href="${p}shop.html">Shop</a><a href="${p}shop.html">Pre-order now</a><a href="${p}how-it-works.html">How It Works</a><a href="${p}collection.html">Collection</a><a href="${p}ingredients-allergens.html">Ingredients & Allergens</a><a href="${p}reviews.html">Reviews</a><a href="${p}faq.html">FAQs</a><a href="${p}search.html">Search</a><a href="${p}contact.html">Contact</a><a href="${p}account.html">Account</a>
      </nav>
    </div>`;
}
...
function footerHtml() {
  ...
  return `<footer class="footer"><div class="container footer-grid">
    ...
    <div class="stack"><strong>Order</strong><a href="${p}shop.html">Shop</a><a href="${p}shop.html">Pre-order now</a><button class="footer-link-button" type="button" data-cart-open>Cart</button><a href="${p}account.html">Account</a></div>
    ...
    <div class="stack"><strong>About</strong><a href="${p}about.html">About Jake's Bakes</a><a href="${p}contact.html">Contact</a><a href="https://instagram.com/${STORE_CONFIG.instagramHandle.replace("@", "")}" rel="noopener">Instagram</a><a href="https://www.tiktok.com/${STORE_CONFIG.tiktokHandle}" rel="noopener">TikTok</a></div>
...
function renderCartLines(container, compact = false) {
  ...
  container.innerHTML = `<div class="stack">${items.map((item) => `
    <article class="card pad cart-row ${compact ? "cart-row-compact" : ""}">
      ${compact ? "" : `<img src="${boxDetails(item.size, item.productId).image}" alt="${boxDetails(item.size, item.productId).alt}" loading="lazy">`}
      <div class="cart-item-copy">
        <h3>${item.boxSize} Cookie Box</h3>
        <p>${item.title}</p>
        <p><strong>${money(item.price)}</strong> each</p>
        <div class="cart-actions">
          <div class="quantity" aria-label="Quantity for ${item.boxSize} Cookie Box"><button type="button" data-qty-minus="${item.id}" aria-label="Reduce quantity">-</button><span>${item.quantity}</span><button type="button" data-qty-plus="${item.id}" aria-label="Increase quantity">+</button></div>
...
function adminInit() {
  ...
  async function refreshAdmin() {
    ...
    await Promise.allSettled([loadRemoteSiteSettings(), loadRemoteReviews()]);
    renderAdminState();
    renderAdminReviews();
    setMessage("Admin data refreshed.", true);
  }
  ...
  panel.addEventListener("click", async (event) => {
    ...
    if (stateButton) {
      ...
      try {
        const nextState = await adminSetPreorderState(stateButton.dataset.adminState, adminCode);
        renderAdminState();
        setMessage(`Preorders are now ${preorderStateLabel(nextState).toLowerCase()}.`, true);
      } catch {
    ...
    if (deleteButton) {
      ...
      try {
        await adminDeleteReview(deleteButton.dataset.adminDeleteReview, adminCode);
        renderAdminReviews();
        setMessage("Review removed.", true);
      } catch {
...
function orderConfirmationInit() {
  ...
  if (!order) {
    target.innerHTML = `<div class="note warning"><strong>No preorder request is available yet.</strong><p>Add a box to your cart, choose a collection slot and send your preorder request to see your confirmation.</p></div><a class="button" href="shop.html">Pre-order now</a>`;
    return;
  }
  target.innerHTML = `<div class="card pad stack"><h2>Request ${order.number}</h2>${order.items.map((item) => `<p>${item.quantity} x ${item.boxSize} Cookie Box - ${escapeHtml(item.title || "Cookie Box")} - ${money(item.price)} each</p>`).join("")}<p><strong>Estimated subtotal: ${order.total}</strong></p><p>Collection date: ${order.collectionDate}</p><p>Collection slot: ${order.slot}</p><p>Jake will reply to ${escapeHtml(order.email)} to confirm availability, payment and collection details.</p><div class="cluster"><a class="button" href="collection.html">Collection information</a><a class="button secondary" href="ingredients-allergens.html">Allergen information</a><a class="button secondary" href="contact.html">Contact Jake</a></div></div>`;
}
...
function searchInit() {
  ...
  const pages = [
    ["Shop", "Choose a Brown Butter Chocolate Chip or Biscoff Royale Cookie Box.", "shop.html"],
    ["Biscoff Royale", "The Biscoff Royale Cookie Box.", "product.html"],
    ["Chocolate Chip", "Brown Butter Chocolate Chip Cookie Box.", "chocolate-chip.html"],
