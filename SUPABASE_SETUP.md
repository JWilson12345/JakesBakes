...
create table if not exists public.reviews (
  ...
  photo_url text,
  photo_name text,
  product_id text not null default 'biscoff-royale',
  product_name text not null default 'The Biscoff Royale',
...
If you already created the reviews table before the Caramelised Biscoff Brownie was added, run this extra SQL first:

```sql
alter table public.reviews
add column if not exists product_id text not null default 'biscoff-royale',
add column if not exists product_name text not null default 'The Biscoff Royale';
```

After those columns exist, set `productReviewColumns` to `true`. Until then, leave it as `false`; the website will still separate reviews by storing a hidden product marker inside the review text.
...
## Marketing reminder signup setup

This powers `signup.html`, the "Remind me" link, and the admin export tools. It uses your existing Supabase free project.
...
create table if not exists public.marketing_signups (
  ...
  phone_marketing boolean not null default false,
...
create or replace function public.save_marketing_signup(
  full_name text,
  email_address text,
  phone_number text,
  wants_email boolean,
  wants_phone boolean,
  signup_consent_text text,
  signup_source text
)
returns public.marketing_signups
language plpgsql
security definer
set search_path = public
as $$
declare
  saved_signup public.marketing_signups;
  clean_email text := nullif(lower(trim(email_address)), '');
  clean_phone text := nullif(trim(phone_number), '');
begin
  if char_length(trim(full_name)) < 1 then
  ...
  select *
  into saved_signup
  from public.marketing_signups
  where (clean_email is not null and lower(email) = clean_email)
     or (clean_phone is not null and phone = clean_phone)
  order by updated_at desc
  limit 1;

  if saved_signup.id is null then
    insert into public.marketing_signups (
      full_name, email, phone, email_marketing, phone_marketing, consent_text, source
    )
    values (
      trim(full_name), clean_email, clean_phone, wants_email, wants_phone, signup_consent_text, signup_source
    )
    returning * into saved_signup;
  else
    update public.marketing_signups
    set full_name = trim(full_name),
        email = coalesce(clean_email, email),
        phone = coalesce(clean_phone, phone),
        email_marketing = wants_email,
        phone_marketing = wants_phone,
        consent_text = signup_consent_text,
        source = signup_source,
        updated_at = now()
    where id = saved_signup.id
    returning * into saved_signup;
  end if;
...
grant execute on function public.save_marketing_signup(text, text, text, boolean, boolean, text, text) to anon;

create or replace function public.admin_list_marketing_signups(admin_code text)
returns setof public.marketing_signups
language plpgsql
security definer
set search_path = public
as $$
...
grant execute on function public.admin_list_marketing_signups(text) to anon;
```

Then confirm `assets/js/config.js` contains:
...
The website stores who opted into email and/or phone/SMS. To send messages:

1. Open `admin.html`.
2. Unlock with `1234`.
3. Use **Copy emails**, **Copy phones** or **Download CSV**.
4. Only message people through the channel they opted into.
5. Include a simple opt-out line, for example: "Reply STOP or use the reminder page to opt out."

Email can be sent for free by importing the CSV into a free email marketing tool, or by copying the email opt-in list into BCC for a very small list. SMS/phone broadcasting is usually not free, so the site stores phone consent and lets you export the numbers, but you may need a paid SMS provider if you want automated text blasts.
